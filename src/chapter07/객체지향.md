...
- 절차 은닉(Process Hiding) : 클래스 내부에 정의 되는 메소드와 관련된 정보와 처리 단계를 은닉하는 방법
...
#### 8-3) 일반 제어자(General Modefier)
- static : class의 field method에 정의되면 객체를 생성하지 않고 직접 클래스명을 통해 접근이 가능함.
...
** final이 붙은 변수는 상수라고 명명함, 보통 static과 대문자로 정의!!
** final이 붙은 클래스는 상속을 하지 않는 마지막 클래스로 정의!!
