...
//		LunchMenu menu = new LunchMenu();
	...
	public void selectLunchMenu() {
		...
		if(system.scan.hasNextInt()) {
			lunchMenuChecke(system.scan.nextInt());
		} else {
			System.out.println("=> 입력 값이 바르지 않음. 다시 입력!!!");
			system.scan.next();
			selectLunchMenu();
		}
	}
	 ...
	 * 메인메뉴 출력
	 */
	public void showMainMenu() { // 시스템 메뉴 : MainMenu
		System.out.println("******************************************");
...
//		createLunchMenu();
	...
	public void selectMainMenu() {
		...
		if(system.scan.hasNextInt()) {
			mainMenuChecke(system.scan.nextInt());			
		} else {
			System.out.println("=> 입력 값이 바르지 않음. 다시 입력!!!");
			system.scan.next();
			selectMainMenu();
	 ...
	 * 메인메뉴 체크
	 */
	public void mainMenuChecke(int mainMenu) {
		switch(mainMenu) {
	 ...
	 * 런치메뉴 체크
	 */
	public void lunchMenuChecke(int lunchMenu) {
		//lunchMenu : 1~4 => 주문가능
		//다른번호 : 메뉴 준비중 = 다시 입력
