		...
		while(menuFlag) {
			...
			switch(menu) {
				case 1: //1. 메뉴 주문
					...
					while(orderFlag) {
						...
						if(scan.hasNextInt()) {
							...
							//주문 갯수 확인
							if(count == MAX_SIZE) {
								System.out.println("주문은 최대"+ MAX_SIZE +"3개까지 가능합니다");
								orderFlag = false;
							} else {
								System.out.print("계속 주문?(계속 : 아무키, 종료(n))");									
				...
				case 3: //음식 결제
					...
					while(paymentFalg) { // 결제 요금 부족시 반복 실행
						...
						paymentMenuList[paymentCount] = orderMenuList[0]+"등..";	//결제메뉴
						totalAmountList[paymentCount] = totalPayment;		   	//결제금액
						paidAmountList[paymentCount] = charge;		 		  	//입금액
