 ...
 * - 학생 및 점수 삭제 메뉴를 추가한다. (검색 + count값 수정)
 * 
...
public class ScoreTestVer3 {
	...
	public static void main(String[] args) {
		...
		while(menuFlag) {
			...
			} else if(menu == 4) { //수정한 부분
				// 1. 수정할 학생명이 존재여부 검색 : 유 -> 새로운 성정 입력 후 수정
				// 2. 수정할 학생명이 존재여부 검색 : 무 -> 검색 데이터 존재X, 반복진행
				if(count != 0) { //데이터 등록 여부 체크
					...
					while (modeFlag){
						...
						if(modiIdx == -1) {
							...
							System.out.println("----------------------------------------------");
							System.out.println("계속 수정 하시겠습니까? (계속: 아무키나 입력 해주세요. 종료:n >");
							if(scan.next().equals("n") ) {
								modeFlag = false; // or break;
							} //if
						}
					} // while
				} // if
				...
				if(count != 0) {
					...
					while(deleteFlag) {
						...
						if(deleteIdx != 1) {
							...
							System.out.println("계속 삭제 하시겠습니까? (계속: 아무키나 입력 해주세요. 종료:n >");
							if(scan.next().equals("n") ) {
					...
					System.out.println("등록된 데이터가 존재하지 않습니다., 다시 입력해주세요> ");					
				} // count if
