...
public class ScoreTestVer4 {
	...
	public static void main(String[] args) {
		...
		int [][] scoreList = new int [MAX_SIZE][]; // 빈 배열의 위치는 scoreList만 사용 가능하다.
		...
		while(menuFlag) {
			...
			if (menu == 1) { // 학생 등록
				...
				for(int i = count; i < nameList.length; i++) {
					...
					// 학생의 점수를 입력할 배열을 생성함
					scoreList[i] = new int [5]; // 국어 영어 수학 총점 평균 값이 들어갈 공간을 다섯개 생성
					int tot = 0;
					int avg = 0;
					for (int j = 0; j < subjectList.length; j++) { //scoreList의 공간만큼 반복 실행
