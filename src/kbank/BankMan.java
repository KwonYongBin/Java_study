...
public class BankMan {
	...
	/**
	 * 고객의 출금정보 유효성 체크 : 고객에게 전달받은 출금용지에 빈갑이 있는지 체크
	 */
	public void validateCheck() {
		System.out.println(this.name+ "고객정보 유효성체크를 진행한다.");
	...
	public void processWithdrawal() {
		...
		if(accountIdx != -1) {
			...
			if(account.getBalance() >= accountPaper.getMoney()) {
				...
				System.out.println(this.name + "잔액부족");
			}
		} else {
			//고객 정보가 일치하지 않음
	...
	public AccountVo[] getAcountList() {
		return accountList;
	}
	public void setAcountList(AccountVo[] acountList) {
